...
class signup_and_signin(TestCase):
    ...
    def test_second_form_get_with_data(self):
        # Setup test
        response = self.client.get(reverse('web_signup_second_form'),
                                   data={'selected_university': 1, 'selected_faculty': 1, 'selected_department': 1})
        # Exercise test
        # Assert test
        self.assertEqual(200, response.status_code)
    ...
    def test_second_form_post_with_data(self):
        ...
        # Exercise test
        response = self.client.post(reverse('web_signup_second_form'),
                                    data={'username': 'test', 'first_name': 'ibrahem', 'last_name': 'amer',
                                          'email': 'ibrahem@hotmail.com', 'password': '12345678abc',
                                          'password_confirm': '12345678abc'})
